...
function PrincipleTailoringContent() {
  ...
  const behaviors = [
    { b: "Processes are tailored to fit the project context, not applied by default", s: "🟢", sBg: C.badgeGreenBg, sFg: C.badgeGreenFg, st: "Doing" },
    { b: "Tailoring decisions are documented with rationale", s: "🟢", sBg: C.badgeGreenBg, sFg: C.badgeGreenFg, st: "Doing" },
    { b: "Tailoring outcomes are reviewed and adjusted based on results", s: "🟢", sBg: C.badgeGreenBg, sFg: C.badgeGreenFg, st: "Doing" },
    { b: "The team understands why processes were tailored (not just what changed)", s: "🟢", sBg: C.badgeGreenBg, sFg: C.badgeGreenFg, st: "Doing" },
    { b: "Tailoring considers organizational culture and maturity", s: "🟢", sBg: C.badgeGreenBg, sFg: C.badgeGreenFg, st: "Doing" },
    { b: "Over-tailoring (too much process) and under-tailoring (too little) are both monitored", s: "🟢", sBg: C.badgeGreenBg, sFg: C.badgeGreenFg, st: "Doing" },
    { b: "Tailoring decisions are revisited when project context changes significantly", s: "🟢", sBg: C.badgeGreenBg, sFg: C.badgeGreenFg, st: "Doing" },
  ];

  const renderAll = () => {
    const titleBanner = (<table style={{ ...S.tbl, marginBottom: "0px" }}><tbody><tr><td style={{ backgroundColor: accent, color: C.white, padding: "16px 20px", fontSize: "22px", fontWeight: 800, fontFamily: S.font, letterSpacing: "0.04em", borderBottom: "4px solid #F59E0B", textAlign: "center" as const }}>✂️ PRINCIPLE 7: TAILORING</td></tr><tr><td style={{ backgroundColor: accentDark, color: C.white, padding: "6px 20px", fontSize: "11px", fontWeight: 600, fontFamily: S.font, textAlign: "center" as const, letterSpacing: "0.08em", textTransform: "uppercase" as const }}>ExecNoteShop &nbsp;|&nbsp; PMBOK® 7 Alignment Pack &nbsp;|&nbsp; Principle Practice Page</td></tr><tr><td style={descStyle}><strong style={{ fontStyle: "italic" }}>“Tailor the delivery approach based on context.”</strong> One size never fits all. Effective PMs deliberately adapt processes, tools, and techniques to fit the unique needs of each project.</td></tr></tbody></table>);

    const header = (<div ref={headerRef} style={{ marginBottom: "12px" }}><table style={S.tbl}><tbody><tr><td style={{ ...S.tdLabel, width: "18%" }}>Project Name</td><td style={{ ...S.td0, width: "32%" }}>[Project Name]</td><td style={{ ...S.tdLabel, width: "18%" }}>Assessment Date</td><td style={{ ...S.td0, width: "32%" }}>[MM/DD/YYYY]</td></tr><tr><td style={S.tdLabelAlt}>Assessed By</td><td style={S.tdAlt}>[Name, PMP]</td><td style={S.tdLabelAlt}>Current Score</td><td style={S.tdAlt}><span style={S.badge("#DDD6FE", accentDark)}>[5/5 — Exemplary]</span></td></tr></tbody></table><CopyButton targetRef={headerRef} label="Copy Section" /></div>);

    const check = (<div ref={checkRef} style={{ marginBottom: "12px" }}><table style={{ ...S.tbl, marginBottom: "0px" }}><tbody><tr><td style={S.sectionBanner(accent)}>✅ TAILORING BEHAVIOR CHECKLIST</td></tr></tbody></table><CopyButton targetRef={checkRef} label="Copy Section" /><table style={S.tbl}><thead><tr><th style={{ ...S.thPrimary, backgroundColor: accent, width: "5%", textAlign: "center" as const }}>✔</th><th style={{ ...S.thPrimary, backgroundColor: accent }}>Behavior / Practice</th><th style={{ ...S.thPrimary, backgroundColor: accent, width: "8%", textAlign: "center" as const }}>Status</th></tr></thead><tbody>{behaviors.map((r, i) => { const bg = i % 2 === 1 ? C.rowAlt : C.white; return (<tr key={i}><td style={{ ...S.td0, backgroundColor: bg, textAlign: "center" as const }}><span style={S.badge(r.sBg, r.sFg)}>{r.s}</span></td><td style={{ ...S.td0, backgroundColor: bg, fontSize: "11px" }}>{r.b}</td><td style={{ ...S.td0, backgroundColor: bg, textAlign: "center" as const }}><span style={S.badge(r.sBg, r.sFg)}>{r.st}</span></td></tr>); })}</tbody></table></div>);

    const evidence = (<div ref={evidenceRef} style={{ marginBottom: "12px" }}><table style={{ ...S.tbl, marginBottom: "0px" }}><tbody><tr><td style={S.sectionBanner(accentDark)}>📋 EVIDENCE &amp; EXAMPLES</td></tr></tbody></table><CopyButton targetRef={evidenceRef} label="Copy Section" /><table style={S.tbl}><tbody><tr><td style={{ ...S.tdLabel, width: "28%" }}>Tailoring Decisions</td><td style={S.td0}>[8 documented decisions in Tailoring Decisions Log; all with rationale and outcomes]</td></tr><tr><td style={S.tdLabelAlt}>Outcome Review</td><td style={S.tdAlt}>[1 decision revised based on outcomes (change control threshold); demonstrates learning loop]</td></tr><tr><td style={S.tdLabel}>Team Understanding</td><td style={S.td0}>[Tailoring rationale shared in sprint planning; team understands why hybrid approach chosen]</td></tr><tr><td style={S.tdLabelAlt}>Areas for Improvement</td><td style={S.tdAlt}>[Continue monitoring outcomes; share tailoring lessons with PMO for organizational learning]</td></tr></tbody></table></div>);

    const actions = (<div ref={actionsRef} style={{ marginBottom: "12px" }}><table style={{ ...S.tbl, marginBottom: "0px" }}><tbody><tr><td style={S.sectionBanner(accent)}>⚡ IMPROVEMENT ACTIONS</td></tr></tbody></table><CopyButton targetRef={actionsRef} label="Copy Section" /><table style={S.tbl}><thead><tr><th style={{ ...S.thSecondary, width: "5%", textAlign: "center" as const }}>#</th><th style={S.thSecondary}>Action</th><th style={{ ...S.thSecondary, width: "12%" }}>Owner</th><th style={{ ...S.thSecondary, width: "10%" }}>Target</th></tr></thead><tbody>{[{ act: "[Share tailoring lessons learned with PMO for organizational knowledge base]", owner: "[PM]", target: "[Quarterly]" }, { act: "[Review all tailoring decisions at phase gate for continued appropriateness]", owner: "[PM]", target: "[Phase 2 gate]" }].map((r, i) => { const bg = i % 2 === 1 ? C.rowAlt : C.white; return (<tr key={i}><td style={{ ...S.td0, backgroundColor: bg, textAlign: "center" as const, fontWeight: 700, color: accent }}>{i + 1}</td><td style={{ ...S.td0, backgroundColor: bg, fontSize: "11px" }}>{r.act}</td><td style={{ ...S.td0, backgroundColor: bg, fontSize: "10px" }}>{r.owner}</td><td style={{ ...S.td0, backgroundColor: bg, fontSize: "10px" }}>{r.target}</td></tr>); })}</tbody></table></div>);

    const footer = (<table style={{ ...S.tbl, marginTop: "8px" }}><tbody><tr><td style={{ backgroundColor: accent, color: "#F5F3FF", padding: "8px 20px", fontSize: "10px", fontFamily: S.font, textAlign: "center" as const, letterSpacing: "0.06em" }}>ExecNoteShop • PMBOK® 7 Alignment Pack • Principle 7 of 12 • © 2026</td></tr></tbody></table>);
...
export default function PrincipleTailoringPage() {
  return (<ThemeProvider><PrincipleTailoringContent /></ThemeProvider>);
}
