...
function PrincipleAdaptabilityContent() {
  ...
  const behaviors = [
    { b: "The project can adapt to changes in scope, schedule, or resources without derailing", s: "🟢", sBg: C.badgeGreenBg, sFg: C.badgeGreenFg, st: "Doing" },
    { b: "The delivery approach supports iterative learning and course correction", s: "🟢", sBg: C.badgeGreenBg, sFg: C.badgeGreenFg, st: "Doing" },
    { b: "The team recovers quickly from setbacks and unexpected events", s: "🟢", sBg: C.badgeGreenBg, sFg: C.badgeGreenFg, st: "Doing" },
    { b: "Lessons learned are applied in real-time, not just at project closure", s: "🟢", sBg: C.badgeGreenBg, sFg: C.badgeGreenFg, st: "Doing" },
    { b: "Retrospectives lead to meaningful process improvements", s: "🟢", sBg: C.badgeGreenBg, sFg: C.badgeGreenFg, st: "Doing" },
    { b: "The team embraces change as normal, not disruptive", s: "🟢", sBg: C.badgeGreenBg, sFg: C.badgeGreenFg, st: "Doing" },
    { b: "Contingency plans exist for known risks and key-person dependencies", s: "🟡", sBg: C.badgeAmberBg, sFg: C.badgeAmberFg, st: "Improve" },
    { b: "Knowledge is distributed — no single point of failure in the team", s: "🟡", sBg: C.badgeAmberBg, sFg: C.badgeAmberFg, st: "Improve" },
  ];

  const renderAll = () => {
    const titleBanner = (<table style={{ ...S.tbl, marginBottom: "0px" }}><tbody><tr><td style={{ backgroundColor: accent, color: C.white, padding: "16px 20px", fontSize: "22px", fontWeight: 800, fontFamily: S.font, letterSpacing: "0.04em", borderBottom: "4px solid #F59E0B", textAlign: "center" as const }}>🔄 PRINCIPLE 11: ADAPTABILITY &amp; RESILIENCE</td></tr><tr><td style={{ backgroundColor: accentDark, color: C.white, padding: "6px 20px", fontSize: "11px", fontWeight: 600, fontFamily: S.font, textAlign: "center" as const, letterSpacing: "0.08em", textTransform: "uppercase" as const }}>ExecNoteShop &nbsp;|&nbsp; PMBOK® 7 Alignment Pack &nbsp;|&nbsp; Principle Practice Page</td></tr><tr><td style={descStyle}><strong style={{ fontStyle: "italic" }}>“Build adaptability and resiliency into the organization and project.”</strong> The ability to respond to change and recover from setbacks is a competitive advantage. Resilient projects are designed to bend, not break.</td></tr></tbody></table>);
    ...
    const check = (<div ref={checkRef} style={{ marginBottom: "12px" }}><table style={{ ...S.tbl, marginBottom: "0px" }}><tbody><tr><td style={S.sectionBanner(accent)}>✅ ADAPTABILITY BEHAVIOR CHECKLIST</td></tr></tbody></table><CopyButton targetRef={checkRef} label="Copy Section" /><table style={S.tbl}><thead><tr><th style={{ ...S.thPrimary, backgroundColor: accent, width: "5%", textAlign: "center" as const }}>✔</th><th style={{ ...S.thPrimary, backgroundColor: accent }}>Behavior / Practice</th><th style={{ ...S.thPrimary, backgroundColor: accent, width: "8%", textAlign: "center" as const }}>Status</th></tr></thead><tbody>{behaviors.map((r, i) => { const bg = i % 2 === 1 ? C.rowAlt : C.white; return (<tr key={i}><td style={{ ...S.td0, backgroundColor: bg, textAlign: "center" as const }}><span style={S.badge(r.sBg, r.sFg)}>{r.s}</span></td><td style={{ ...S.td0, backgroundColor: bg, fontSize: "11px" }}>{r.b}</td><td style={{ ...S.td0, backgroundColor: bg, textAlign: "center" as const }}><span style={S.badge(r.sBg, r.sFg)}>{r.st}</span></td></tr>); })}</tbody></table></div>);
    const evidence = (<div ref={evidenceRef} style={{ marginBottom: "12px" }}><table style={{ ...S.tbl, marginBottom: "0px" }}><tbody><tr><td style={S.sectionBanner(accentDark)}>📋 EVIDENCE &amp; EXAMPLES</td></tr></tbody></table><CopyButton targetRef={evidenceRef} label="Copy Section" /><table style={S.tbl}><tbody><tr><td style={{ ...S.tdLabel, width: "28%" }}>Adaptation Examples</td><td style={S.td0}>[Vendor API delay → team created mock API within 24 hours; hybrid approach allows sprint-level flexibility]</td></tr><tr><td style={S.tdLabelAlt}>Resilience Evidence</td><td style={S.tdAlt}>[Key developer sick for 1 week → pair programming backlog enabled seamless coverage; velocity maintained]</td></tr><tr><td style={S.tdLabel}>Real-Time Learning</td><td style={S.td0}>[Retro actions implemented same sprint; focus blocks adopted from Sprint 3 feedback]</td></tr><tr><td style={S.tdLabelAlt}>Areas for Improvement</td><td style={S.tdAlt}>[Bus factor of 1 for data migration expertise; need cross-training plan; contingency plans for key-person dependencies]</td></tr></tbody></table></div>);
    const actions = (<div ref={actionsRef} style={{ marginBottom: "12px" }}><table style={{ ...S.tbl, marginBottom: "0px" }}><tbody><tr><td style={S.sectionBanner(accent)}>⚡ IMPROVEMENT ACTIONS</td></tr></tbody></table><CopyButton targetRef={actionsRef} label="Copy Section" /><table style={S.tbl}><thead><tr><th style={{ ...S.thSecondary, width: "5%", textAlign: "center" as const }}>#</th><th style={S.thSecondary}>Action</th><th style={{ ...S.thSecondary, width: "12%" }}>Owner</th><th style={{ ...S.thSecondary, width: "10%" }}>Target</th></tr></thead><tbody>{[{ act: "[Create cross-training plan to eliminate single points of failure (data migration, SSO)]", owner: "[Tech Lead]", target: "[Sprint 5]" }, { act: "[Document contingency plans for top 3 key-person dependencies]", owner: "[PM]", target: "[Week 9]" }, { act: "[Add “bus factor” column to skills matrix and target ≥ 2 for all critical skills]", owner: "[PM]", target: "[Week 8]" }].map((r, i) => { const bg = i % 2 === 1 ? C.rowAlt : C.white; return (<tr key={i}><td style={{ ...S.td0, backgroundColor: bg, textAlign: "center" as const, fontWeight: 700, color: accent }}>{i + 1}</td><td style={{ ...S.td0, backgroundColor: bg, fontSize: "11px" }}>{r.act}</td><td style={{ ...S.td0, backgroundColor: bg, fontSize: "10px" }}>{r.owner}</td><td style={{ ...S.td0, backgroundColor: bg, fontSize: "10px" }}>{r.target}</td></tr>); })}</tbody></table></div>);
    const footer = (<table style={{ ...S.tbl, marginTop: "8px" }}><tbody><tr><td style={{ backgroundColor: accent, color: "#F5F3FF", padding: "8px 20px", fontSize: "10px", fontFamily: S.font, textAlign: "center" as const, letterSpacing: "0.06em" }}>ExecNoteShop • PMBOK® 7 Alignment Pack • Principle 11 of 12 • © 2026</td></tr></tbody></table>);
...
export default function PrincipleAdaptabilityPage() {
  return (<ThemeProvider><PrincipleAdaptabilityContent /></ThemeProvider>);
}
