...
function PrincipleChangeContent() {
  ...
  const behaviors = [
    { b: "A structured change management approach exists for the project", s: "🟢", sBg: C.badgeGreenBg, sFg: C.badgeGreenFg, st: "Doing" },
    { b: "Stakeholders affected by change are identified and engaged early", s: "🟡", sBg: C.badgeAmberBg, sFg: C.badgeAmberFg, st: "Improve" },
    { b: "Change champions are identified and activated within affected groups", s: "🟡", sBg: C.badgeAmberBg, sFg: C.badgeAmberFg, st: "Improve" },
    { b: "Training and support are planned for users adopting new processes/tools", s: "🟢", sBg: C.badgeGreenBg, sFg: C.badgeGreenFg, st: "Doing" },
    { b: "Resistance to change is anticipated, detected, and addressed", s: "🟡", sBg: C.badgeAmberBg, sFg: C.badgeAmberFg, st: "Improve" },
    { b: "Communication about change is proactive, clear, and consistent", s: "🟢", sBg: C.badgeGreenBg, sFg: C.badgeGreenFg, st: "Doing" },
    { b: "The “why” behind changes is articulated, not just the “what”", s: "🟢", sBg: C.badgeGreenBg, sFg: C.badgeGreenFg, st: "Doing" },
    { b: "Adoption and sustainability of changes are measured post-implementation", s: "🟡", sBg: C.badgeAmberBg, sFg: C.badgeAmberFg, st: "Improve" },
  ];

  const renderAll = () => {
    const titleBanner = (<table style={{ ...S.tbl, marginBottom: "0px" }}><tbody><tr><td style={{ backgroundColor: accent, color: C.white, padding: "16px 20px", fontSize: "22px", fontWeight: 800, fontFamily: S.font, letterSpacing: "0.04em", borderBottom: "4px solid #F59E0B", textAlign: "center" as const }}>🔄 PRINCIPLE 12: CHANGE</td></tr><tr><td style={{ backgroundColor: accentDark, color: C.white, padding: "6px 20px", fontSize: "11px", fontWeight: 600, fontFamily: S.font, textAlign: "center" as const, letterSpacing: "0.08em", textTransform: "uppercase" as const }}>ExecNoteShop &nbsp;|&nbsp; PMBOK® 7 Alignment Pack &nbsp;|&nbsp; Principle Practice Page</td></tr><tr><td style={descStyle}><strong style={{ fontStyle: "italic" }}>“Enable change to achieve the envisioned future state.”</strong> Projects are vehicles for change. Success means not just delivering outputs, but ensuring the organization can adopt, sustain, and benefit from the changes delivered.</td></tr></tbody></table>);
    ...
    const check = (<div ref={checkRef} style={{ marginBottom: "12px" }}><table style={{ ...S.tbl, marginBottom: "0px" }}><tbody><tr><td style={S.sectionBanner(accent)}>✅ CHANGE BEHAVIOR CHECKLIST</td></tr></tbody></table><CopyButton targetRef={checkRef} label="Copy Section" /><table style={S.tbl}><thead><tr><th style={{ ...S.thPrimary, backgroundColor: accent, width: "5%", textAlign: "center" as const }}>✔</th><th style={{ ...S.thPrimary, backgroundColor: accent }}>Behavior / Practice</th><th style={{ ...S.thPrimary, backgroundColor: accent, width: "8%", textAlign: "center" as const }}>Status</th></tr></thead><tbody>{behaviors.map((r, i) => { const bg = i % 2 === 1 ? C.rowAlt : C.white; return (<tr key={i}><td style={{ ...S.td0, backgroundColor: bg, textAlign: "center" as const }}><span style={S.badge(r.sBg, r.sFg)}>{r.s}</span></td><td style={{ ...S.td0, backgroundColor: bg, fontSize: "11px" }}>{r.b}</td><td style={{ ...S.td0, backgroundColor: bg, textAlign: "center" as const }}><span style={S.badge(r.sBg, r.sFg)}>{r.st}</span></td></tr>); })}</tbody></table></div>);
    const evidence = (<div ref={evidenceRef} style={{ marginBottom: "12px" }}><table style={{ ...S.tbl, marginBottom: "0px" }}><tbody><tr><td style={S.sectionBanner(accentDark)}>📋 EVIDENCE & EXAMPLES</td></tr></tbody></table><CopyButton targetRef={evidenceRef} label="Copy Section" /><table style={S.tbl}><tbody><tr><td style={{ ...S.tdLabel, width: "28%" }}>Change Approach</td><td style={S.td0}>[Change management plan exists; communications scheduled; training plan for sales team in place]</td></tr><tr><td style={S.tdLabelAlt}>Champion Network</td><td style={S.tdAlt}>[Just starting — 2 champions identified in sales; need to expand to operations and finance]</td></tr><tr><td style={S.tdLabel}>Resistance Management</td><td style={S.td0}>[Sales Ops resistance identified reactively; need earlier detection mechanisms and playbooks]</td></tr><tr><td style={S.tdLabelAlt}>Adoption Measurement</td><td style={S.tdAlt}>[Pipeline dashboard: 85% daily active usage = strong adoption; need similar metrics for future features]</td></tr><tr><td style={S.tdLabel}>Areas for Improvement</td><td style={S.td0}>[Activate champion network broadly; build resistance detection playbook; measure adoption for all features]</td></tr></tbody></table></div>);
    const actions = (<div ref={actionsRef} style={{ marginBottom: "12px" }}><table style={{ ...S.tbl, marginBottom: "0px" }}><tbody><tr><td style={S.sectionBanner(accent)}>⚡ IMPROVEMENT ACTIONS</td></tr></tbody></table><CopyButton targetRef={actionsRef} label="Copy Section" /><table style={S.tbl}><thead><tr><th style={{ ...S.thSecondary, width: "5%", textAlign: "center" as const }}>#</th><th style={S.thSecondary}>Action</th><th style={{ ...S.thSecondary, width: "12%" }}>Owner</th><th style={{ ...S.thSecondary, width: "10%" }}>Target</th></tr></thead><tbody>{[{ act: "[Expand champion network to operations and finance — identify 2 champions per group]", owner: "[PM + Change Lead]", target: "[Week 8]" }, { act: "[Create resistance detection playbook with early warning indicators and responses]", owner: "[Change Lead]", target: "[Week 9]" }, { act: "[Define adoption metrics and measurement cadence for all upcoming feature releases]", owner: "[PM + BA]", target: "[Sprint 5]" }, { act: "[Schedule “change readiness” check before each major feature release]", owner: "[PM]", target: "[Ongoing]" }].map((r, i) => { const bg = i % 2 === 1 ? C.rowAlt : C.white; return (<tr key={i}><td style={{ ...S.td0, backgroundColor: bg, textAlign: "center" as const, fontWeight: 700, color: accent }}>{i + 1}</td><td style={{ ...S.td0, backgroundColor: bg, fontSize: "11px" }}>{r.act}</td><td style={{ ...S.td0, backgroundColor: bg, fontSize: "10px" }}>{r.owner}</td><td style={{ ...S.td0, backgroundColor: bg, fontSize: "10px" }}>{r.target}</td></tr>); })}</tbody></table></div>);
    const footer = (<table style={{ ...S.tbl, marginTop: "8px" }}><tbody><tr><td style={{ backgroundColor: accent, color: "#F5F3FF", padding: "8px 20px", fontSize: "10px", fontFamily: S.font, textAlign: "center" as const, letterSpacing: "0.06em" }}>ExecNoteShop • PMBOK® 7 Alignment Pack • Principle 12 of 12 • © 2026</td></tr></tbody></table>);
...
export default function PrincipleChangePage() {
  return (<ThemeProvider><PrincipleChangeContent /></ThemeProvider>);
}
