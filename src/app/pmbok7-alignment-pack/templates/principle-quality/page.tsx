...
function PrincipleQualityContent() {
  ...
  const behaviors = [
    { b: "Quality standards are defined and understood by the team (DoD, acceptance criteria)", s: "🟢", sBg: C.badgeGreenBg, sFg: C.badgeGreenFg, st: "Doing" },
    { b: "Quality is built into the process, not just inspected at the end", s: "🟢", sBg: C.badgeGreenBg, sFg: C.badgeGreenFg, st: "Doing" },
    { b: "Code reviews, testing, and UAT are embedded in the delivery cadence", s: "🟢", sBg: C.badgeGreenBg, sFg: C.badgeGreenFg, st: "Doing" },
    { b: "Defect prevention is prioritized over defect detection", s: "🟢", sBg: C.badgeGreenBg, sFg: C.badgeGreenFg, st: "Doing" },
    { b: "Continuous improvement of quality practices is part of retrospectives", s: "🟢", sBg: C.badgeGreenBg, sFg: C.badgeGreenFg, st: "Doing" },
    { b: "Quality metrics are tracked and visible to the team and stakeholders", s: "🟢", sBg: C.badgeGreenBg, sFg: C.badgeGreenFg, st: "Doing" },
    { b: "Technical debt is managed proactively, not ignored", s: "🟡", sBg: C.badgeAmberBg, sFg: C.badgeAmberFg, st: "Improve" },
  ];

  const renderAll = () => {
    const titleBanner = (<table style={{ ...S.tbl, marginBottom: "0px" }}><tbody><tr><td style={{ backgroundColor: accent, color: C.white, padding: "16px 20px", fontSize: "22px", fontWeight: 800, fontFamily: S.font, letterSpacing: "0.04em", borderBottom: "4px solid #F59E0B", textAlign: "center" as const }}>✨ PRINCIPLE 8: QUALITY</td></tr><tr><td style={{ backgroundColor: accentDark, color: C.white, padding: "6px 20px", fontSize: "11px", fontWeight: 600, fontFamily: S.font, textAlign: "center" as const, letterSpacing: "0.08em", textTransform: "uppercase" as const }}>ExecNoteShop &nbsp;|&nbsp; PMBOK® 7 Alignment Pack &nbsp;|&nbsp; Principle Practice Page</td></tr><tr><td style={descStyle}><strong style={{ fontStyle: "italic" }}>“Build quality into processes and deliverables.”</strong> Quality is not a phase — it’s a mindset. Every process, deliverable, and decision should reflect a commitment to excellence and fitness for purpose.</td></tr></tbody></table>);
    ...
    const check = (<div ref={checkRef} style={{ marginBottom: "12px" }}><table style={{ ...S.tbl, marginBottom: "0px" }}><tbody><tr><td style={S.sectionBanner(accent)}>✅ QUALITY BEHAVIOR CHECKLIST</td></tr></tbody></table><CopyButton targetRef={checkRef} label="Copy Section" /><table style={S.tbl}><thead><tr><th style={{ ...S.thPrimary, backgroundColor: accent, width: "5%", textAlign: "center" as const }}>✔</th><th style={{ ...S.thPrimary, backgroundColor: accent }}>Behavior / Practice</th><th style={{ ...S.thPrimary, backgroundColor: accent, width: "8%", textAlign: "center" as const }}>Status</th></tr></thead><tbody>{behaviors.map((r, i) => { const bg = i % 2 === 1 ? C.rowAlt : C.white; return (<tr key={i}><td style={{ ...S.td0, backgroundColor: bg, textAlign: "center" as const }}><span style={S.badge(r.sBg, r.sFg)}>{r.s}</span></td><td style={{ ...S.td0, backgroundColor: bg, fontSize: "11px" }}>{r.b}</td><td style={{ ...S.td0, backgroundColor: bg, textAlign: "center" as const }}><span style={S.badge(r.sBg, r.sFg)}>{r.st}</span></td></tr>); })}</tbody></table></div>);
    const evidence = (<div ref={evidenceRef} style={{ marginBottom: "12px" }}><table style={{ ...S.tbl, marginBottom: "0px" }}><tbody><tr><td style={S.sectionBanner(accentDark)}>📋 EVIDENCE &amp; EXAMPLES</td></tr></tbody></table><CopyButton targetRef={evidenceRef} label="Copy Section" /><table style={S.tbl}><tbody><tr><td style={{ ...S.tdLabel, width: "28%" }}>Quality Built In</td><td style={S.td0}>[Code reviews 100%; unit test coverage at 82%; UAT per sprint; 0 production defects]</td></tr><tr><td style={S.tdLabelAlt}>Metrics Tracked</td><td style={S.tdAlt}>[Defect escape rate, rework rate, UAT acceptance rate all on dashboard]</td></tr><tr><td style={S.tdLabel}>Prevention Focus</td><td style={S.td0}>[DoD enforced at story level; acceptance criteria reviewed before development starts]</td></tr><tr><td style={S.tdLabelAlt}>Technical Debt</td><td style={S.tdAlt}>[Backlog has 6 tech debt items; need to allocate sprint capacity for addressing them]</td></tr></tbody></table></div>);
    const actions = (<div ref={actionsRef} style={{ marginBottom: "12px" }}><table style={{ ...S.tbl, marginBottom: "0px" }}><tbody><tr><td style={S.sectionBanner(accent)}>⚡ IMPROVEMENT ACTIONS</td></tr></tbody></table><CopyButton targetRef={actionsRef} label="Copy Section" /><table style={S.tbl}><thead><tr><th style={{ ...S.thSecondary, width: "5%", textAlign: "center" as const }}>#</th><th style={S.thSecondary}>Action</th><th style={{ ...S.thSecondary, width: "12%" }}>Owner</th><th style={{ ...S.thSecondary, width: "10%" }}>Target</th></tr></thead><tbody>{[{ act: "[Allocate 15% of Sprint 5 capacity to technical debt reduction]", owner: "[PM + Tech Lead]", target: "[Sprint 5]" }, { act: "[Create technical debt tracking dashboard with severity ratings]", owner: "[Tech Lead]", target: "[Week 8]" }, { act: "[Increase unit test coverage target from 82% to 90%]", owner: "[Dev Team]", target: "[Sprint 6]" }].map((r, i) => { const bg = i % 2 === 1 ? C.rowAlt : C.white; return (<tr key={i}><td style={{ ...S.td0, backgroundColor: bg, textAlign: "center" as const, fontWeight: 700, color: accent }}>{i + 1}</td><td style={{ ...S.td0, backgroundColor: bg, fontSize: "11px" }}>{r.act}</td><td style={{ ...S.td0, backgroundColor: bg, fontSize: "10px" }}>{r.owner}</td><td style={{ ...S.td0, backgroundColor: bg, fontSize: "10px" }}>{r.target}</td></tr>); })}</tbody></table></div>);
    const footer = (<table style={{ ...S.tbl, marginTop: "8px" }}><tbody><tr><td style={{ backgroundColor: accent, color: "#F5F3FF", padding: "8px 20px", fontSize: "10px", fontFamily: S.font, textAlign: "center" as const, letterSpacing: "0.06em" }}>ExecNoteShop • PMBOK® 7 Alignment Pack • Principle 8 of 12 • © 2026</td></tr></tbody></table>);
...
export default function PrincipleQualityPage() {
  return (<ThemeProvider><PrincipleQualityContent /></ThemeProvider>);
}
