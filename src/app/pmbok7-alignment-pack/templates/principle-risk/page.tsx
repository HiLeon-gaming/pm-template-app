...
function PrincipleRiskContent() {
  ...
  const renderAll = () => {
    ...
    const check = (<div ref={checkRef} style={{ marginBottom: "12px" }}><div style={S.sectionBanner(accent)}>✅ RISK BEHAVIOR CHECKLIST</div><CopyButton targetRef={checkRef} label="Copy Section" /><table style={S.tbl}><thead><tr><th style={{ ...S.thPrimary, backgroundColor: accent, width: "5%", textAlign: "center" as const }}>✔</th><th style={{ ...S.thPrimary, backgroundColor: accent }}>Behavior / Practice</th><th style={{ ...S.thPrimary, backgroundColor: accent, width: "8%", textAlign: "center" as const }}>Status</th></tr></thead><tbody>{behaviors.map((r, i) => { const bg = i % 2 === 1 ? C.rowAlt : C.white; return (<tr key={i}><td style={{ ...S.td0, backgroundColor: bg, textAlign: "center" as const }}><span style={S.badge(r.sBg, r.sFg)}>{r.s}</span></td><td style={{ ...S.td0, backgroundColor: bg, fontSize: "11px" }}>{r.b}</td><td style={{ ...S.td0, backgroundColor: bg, textAlign: "center" as const }}><span style={S.badge(r.sBg, r.sFg)}>{r.st}</span></td></tr>); })}</tbody></table></div>);
    const evidence = (<div ref={evidenceRef} style={{ marginBottom: "12px" }}><div style={S.sectionBanner(accentDark)}>📋 EVIDENCE &amp; EXAMPLES</div><CopyButton targetRef={evidenceRef} label="Copy Section" /><table style={S.tbl}><tbody><tr><td style={{ ...S.tdLabel, width: "28%" }}>Proactive Identification</td><td style={S.td0}>[14 active risks; team raises risks in standups; weekly risk reviews attended by full team]</td></tr><tr><td style={S.tdLabelAlt}>Response Effectiveness</td><td style={S.tdAlt}>[2 risk responses triggered this sprint; both contained within tolerance; mock API workaround successful]</td></tr><tr><td style={S.tdLabel}>Opportunity Management</td><td style={S.td0}>[Focus has been on threats; need to actively identify and exploit positive risks / opportunities]</td></tr><tr><td style={S.tdLabelAlt}>Assumption Testing</td><td style={S.tdAlt}>[2 of 8 assumptions at risk; need formal stress-testing process; currently informal]</td></tr></tbody></table></div>);
